...
import { Lightbulb, Search as SearchIcon, TrendingUp, Zap } from "lucide-react";
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-6xl mx-auto px-4">
      <Card variant="pokeball" className="overflow-hidden">
        <CardHeader className="grid grid-cols-[auto_1fr] grid-rows-2 gap-x-2 gap-y-1">
          ...
          <CardDescription className="col-span-2">
            Explore the most iconic Pokémon and their evolution chains
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-1 gap-4">
            {featuredPokemon.map((pokemon) => (
              ...
              >
                ...
                <Typography as="span" variant="pokeball-caption">
                  #{pokemon.id}
                </Typography>
              </Button>
            ))}
      ...
      <Card variant="greatball" className="overflow-hidden">
        <CardHeader className="grid grid-cols-[auto_1fr] grid-rows-2 gap-x-2 gap-y-1">
          ...
          <CardDescription className="col-span-2">
            Curated Pokémon to explore interesting evolution paths
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-1 gap-4">
            {recommendedPokemon.map((pokemon) => (
              <Button
                ...
                onClick={() => handlePokemonClick(pokemon.id)}
              >
                <Typography as="span" variant="subtitle" className="col-start-1">
                  {pokemon.name}
                </Typography>
                <Typography
                  as="span"
                  variant="greatball-caption"
                  className="col-start-2 row-start-1"
                >
                  {pokemon.id}
                </Typography>
                <Typography as="span" variant="caption" className="col-span-2">
                  {pokemon.description}
                </Typography>
              </Button>
            ))}
      ...
      <Card className="overflow-hidden md:col-span-2">
        <CardHeader className="grid grid-cols-[auto_1fr] grid-rows-2 gap-x-2 gap-y-1">
          ...
          <CardDescription className="col-span-2">
            Learn how to make the most of the Pokémon Evolution Explorer
          </CardDescription>
        </CardHeader>
        <CardContent>
